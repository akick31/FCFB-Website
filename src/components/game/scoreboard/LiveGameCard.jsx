...
// ── Quarter score table ─────────────────────────────────────────
const QuarterScoreTable = ({ homeTeamData, awayTeamData, homeTeamName, awayTeamName, homeStats, awayStats, homeScore, awayScore, quarter }) => {
    const theme = useTheme();
    if (!homeStats && !awayStats) return null;

    // Determine how many OT periods have scores
    const otKeys = ['ot_score', 'ot2_score', 'ot3_score', 'ot4_score'];
    const hasOT = otKeys.some(k => (homeStats?.[k] != null && homeStats[k] !== 0) || (awayStats?.[k] != null && awayStats[k] !== 0));

    const cols = ['Q1', 'Q2', 'Q3', 'Q4'];
    if (hasOT) cols.push('OT');
    cols.push('T');

    const statsVal = (stats, col) => {
        if (!stats) return '-';
        if (col === 'T') return stats.q1_score != null
            ? (stats.q1_score || 0) + (stats.q2_score || 0) + (stats.q3_score || 0) + (stats.q4_score || 0) + (stats.ot_score || 0)
            : '-';
        const map = { Q1: 'q1_score', Q2: 'q2_score', Q3: 'q3_score', Q4: 'q4_score', OT: 'ot_score' };
        const v = stats[map[col]];
        return v != null ? v : '-';
    };
    ...
    const logoOrAbbr = (teamData, name) => (
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
            {teamData?.logo ? (
                <Box sx={{ width: 16, height: 16, flexShrink: 0 }}>
                    <img src={teamData.logo} alt="" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
                </Box>
            ) : null}
            <Typography sx={{ fontSize: '0.68rem', fontWeight: 600, color: 'text.secondary', whiteSpace: 'nowrap', maxWidth: 56, overflow: 'hidden', textOverflow: 'ellipsis' }}>
                {teamData?.abbreviation || name?.substring(0, 4)}
            </Typography>
        </Box>
    );
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                    <tr>
                        ...
                        {cols.map(c => (
                            <td key={c}>
                                <Typography sx={{ ...cellSx(false), color: 'text.disabled', fontSize: '0.62rem' }}>
                                    {c}
                                </Typography>
                            </td>
                        ))}
                ...
                <tbody>
                    {/* Away row */}
                    <tr>
                        <td>{logoOrAbbr(awayTeamData, awayTeamName)}</td>
                        {cols.map(c => (
                            <td key={c}>
                                <Typography sx={cellSx(c === 'T')}>
                                    {statsVal(awayStats, c)}
                                </Typography>
                            </td>
                        ))}
                    </tr>
                    {/* Home row */}
                    <tr>
                        <td>{logoOrAbbr(homeTeamData, homeTeamName)}</td>
                        {cols.map(c => (
                            <td key={c}>
                                <Typography sx={cellSx(c === 'T')}>
                                    {statsVal(homeStats, c)}
                                </Typography>
                            </td>
                        ))}
                    </tr>
